...
-  No pooling          
```{r}
d_hyp = list(length = scale(hypo$length), light = as.factor(hypo$light), geno = as.factor(hypo$line), expe = as.factor(hypo$exp), plate = as.factor(hypo$plate))

m_hypo1 = ulam(
  alist(
    length ~ dnorm(mu_len, sigma),
    mu_len <- b_light[light], b_geno[geno] + b_exp[expe] + b_p[plate],
    b_light[light] ~ dnorm(0, 1), 
    b_geno[geno] ~ dnorm(0,1),
    b_exp[expe] ~ dnorm(0, 1),
    b_p[plate] ~ dnorm(0, 1),
