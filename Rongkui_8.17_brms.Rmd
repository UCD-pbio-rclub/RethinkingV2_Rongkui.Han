...
```{r}
predict(m4.3brm, newdata = data.frame(weight = weight.seq- mean(d$weight)) )
```
...
d4 = list(
  ...
  trt = tomato$trt,
  spe = tomato$species,
  inter = as.factor(paste(tomato$species, tomato$trt, sep = "_"))
)

mSol_ulam <- ulam(
  alist(
    hyp ~ dnorm(mu, sigma),
    mu <- b_t[trt] + b_s[spe] + b_i[inter],
    sigma ~ dexp(1),
    b_t[trt] ~ dnorm(0, 1),
    b_s[spe] ~ dnorm(0, 1),
    b_i[inter] ~ dnorm(0, 1)
...
d5 = list(
  hyp = scale(tomato$hyp),
  trt = as.numeric(tomato$trt),
  spe = as.numeric(tomato$species),
  inter = as.numeric(as.factor(paste(as.numeric(tomato$species), as.numeric(tomato$trt), sep = "_")))
)

get_prior(hyp ~ trt + spe + trt:spe, data=d5)
mSol_brms <- brm(hyp ~ trt + spe + trt:spe, data=d5,
               prior=c(
                 set_prior("normal(0,1)", class = "b", coef = "trt"),
                 set_prior("normal(0,1)", class = "b", coef = "spe"),
                 set_prior("normal(0,1)", class = "b", coef = "trt:spe"),
